# Coursera Data Science Specialization Course 3 Week 4 Assignment Codebook

## Acknowledgement and Disclaimer

Acknowledgement for portions of this Codebook and project goes to the original
...
## Summary
The purpose of this codebook is to satisfy the requirements of the week 4
...
A summary of the original studyt is provided by the original authors in their
readme:
...
From the original authors' readme and codebook files:
...
The script called `run_analysis.R` accesses the original data set, reads it into
R, and assembles it into a single table. The script then extracts only the
columns required for the assignment (measurements on the mean and standard
deviation). The table is then rearranged so it is in a tidy format, and then it
is summarized by taking the means for each variable for each activity and
subject. This table is left as an object available after running the script, it
is also saved to a file called `TidyData.txt`.
